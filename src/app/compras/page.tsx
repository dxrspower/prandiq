...
export default async function PurchasesPage() {
  ...
  const [
    ...
    supabase
      ...
      .select(
        "supplier_id, product_id, purchase_price, minimum_order_quantity, preferred",
      )
      .eq("restaurant_id", membership.restaurant_id)
      .eq("active", true),
    supabase
      ...
      .select(
        "id, supplier_id, order_number, status, order_date, expected_delivery_date, subtotal, total_amount, notes",
      )
      .eq("restaurant_id", membership.restaurant_id)
    ...
    supabase
      ...
      .select(
        "id, purchase_order_id, product_id, quantity, received_quantity, unit_price, line_total",
      )
      .eq("restaurant_id", membership.restaurant_id)
      .order("created_at"),
  ]);
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col border-r border-slate-200 bg-white px-5 py-6 lg:flex">
        ...
        <nav className="mt-9 space-y-1">
          ...
          {menuItems.map(([label, icon, href]) => (
            <Link
              key={label}
              href={href}
              className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold ${label === "Compras" ? "bg-emerald-50 text-emerald-700" : "text-slate-500 hover:bg-slate-50"}`}
            >
              <span
                className={`grid h-7 w-7 place-items-center rounded-lg ${label === "Compras" ? "bg-emerald-600 text-white" : "bg-slate-100"}`}
              >
                {icon}
              </span>
              {label}
        ...
        <div className="mt-auto rounded-2xl bg-slate-900 p-4 text-white">
          ...
          <p className="mt-2 text-sm font-bold">
            Abastece tu inventario desde proveedores.
          </p>
        </div>
      </aside>
      ...
      <section className="lg:pl-64">
        ...
        <div className="mx-auto grid max-w-7xl gap-6 p-5 lg:grid-cols-[360px_1fr]">
          <section className="h-fit rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Crea un borrador para comenzar el pedido.
            </p>
            {suppliers.length ? (
          ...
          <section className="space-y-5">
            {orders.length === 0 ? (
              ...
              orders.map((order) => {
                ...
                const linkedProducts = links.filter(
                  (link) => link.supplier_id === order.supplier_id,
                );
                const canEdit = order.status === "draft";
                const canReceive = ["sent", "partially_received"].includes(
                  order.status,
                );
                    ...
                    <div className="border-t border-slate-200 p-6">
                      ...
                      {canEdit && (
                        <form
                          ...
                          className="mb-6 grid gap-3 rounded-2xl bg-slate-50 p-4 md:grid-cols-[1fr_120px_130px_auto]"
                        >
                          <input
                            type="hidden"
                          ...
                          >
                            ...
                            {linkedProducts.map((link) => {
                              ...
                              return product ? (
                                ...
                                >
                                  {product.name} · S/{" "}
                                  {Number(link.purchase_price).toFixed(2)}
                                </option>
                              ) : null;
                            })}
                          </select>
                          ...
                          <input
                            ...
                            className="rounded-xl border border-slate-200 px-3 py-2.5"
                          />
                          <button className="rounded-xl bg-slate-900 px-4 py-2.5 font-bold text-white hover:bg-slate-800">
                            Añadir
                          </button>
                        </form>
                      )}
                      ...
                      <div className="space-y-3">
                        {orderItems.length === 0 ? (
                          <p className="py-5 text-center text-sm text-slate-500">
                            Añade productos a este borrador.
                          </p>
                        ) : (
                          orderItems.map((item) => {
                              ...
                              >
                                <div className="flex flex-wrap items-center justify-between gap-3">
                                  <div>
                                    ...
                                    <p className="mt-1 text-xs text-slate-500">
                                      Recibido: {item.received_quantity} de{" "}
                                ...
                                {canEdit && (
                                  <div className="mt-3 flex flex-wrap gap-2 border-t border-slate-100 pt-3">
                                    <form
                                      action={updatePurchaseOrderItem}
                                    ...
                                    >
                                      ...
                                      <input
                                        ...
                                        className="w-28 rounded-lg border border-slate-200 px-3 py-2 text-sm"
                                      />
                                      <button className="rounded-lg bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700">
                                        Guardar
                                      </button>
                                    </form>
                                    <form action={removePurchaseOrderItem}>
                                      ...
                                      <input
                                        ...
                                        value={order.id}
                                      />
                                      <button className="rounded-lg bg-rose-50 px-3 py-2 text-sm font-bold text-rose-700">
                                        Quitar
                                      </button>
                                    </form>
                                  </div>
                                ...
                                {canReceive && pending > 0 && (
                                  ...
                                  >
                                    ...
                                    <span className="text-xs text-slate-500">
                                      Máximo pendiente: {pending}{" "}
                                      {product?.unit || ""}
                                    </span>
                                    <button className="ml-auto rounded-lg bg-blue-600 px-4 py-2 text-sm font-bold text-white hover:bg-blue-700">
                                      Confirmar recepción
                                    </button>
                                  </form>
                                )}
                        ...
                        {canEdit && (
                          <form action={sendPurchaseOrder}>
                            <input
                              ...
                              value={order.id}
                            />
                            <button className="rounded-xl bg-blue-600 px-5 py-2.5 font-bold text-white hover:bg-blue-700">
                              Marcar como enviada
                            </button>
                          </form>
                        )}
                        {["draft", "sent"].includes(order.status) && (
                          <form action={cancelPurchaseOrder}>
                            <input
                              ...
                              value={order.id}
                            />
                            <button className="rounded-xl bg-rose-50 px-5 py-2.5 font-bold text-rose-700 hover:bg-rose-100">
                              Cancelar orden
                            </button>
                          </form>
                        )}
